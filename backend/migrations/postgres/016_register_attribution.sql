...
-- and, worse, could not open a second drawer at all: 011's constraint allows
-- only one open cash drawer for the *entire installation*, not one per
-- register.
--
...
--
-- Timestamps stay TIMESTAMP/TIMESTAMPTZ, never BIGINT: `pg` returns int8 as a
-- string, and the adapters convert with `new Date(row.x).getTime()`, which
-- yields Invalid Date on a string of digits. None of the columns added here
-- are timestamps, but the backfill below joins against `opened_at`, which is
-- — this note exists so a future column addition here doesn't get it wrong.

ALTER TABLE orders ADD COLUMN IF NOT EXISTS register_id UUID REFERENCES registers(id);
ALTER TABLE orders ADD COLUMN IF NOT EXISTS cashier_user_id UUID REFERENCES users(id);
ALTER TABLE orders ADD COLUMN IF NOT EXISTS drawer_session_id UUID REFERENCES cash_drawer_sessions(id);
ALTER TABLE orders ADD COLUMN IF NOT EXISTS override_by_user_id UUID REFERENCES users(id);

ALTER TABLE returns ADD COLUMN IF NOT EXISTS register_id UUID REFERENCES registers(id);
ALTER TABLE returns ADD COLUMN IF NOT EXISTS cashier_user_id UUID REFERENCES users(id);
ALTER TABLE returns ADD COLUMN IF NOT EXISTS override_by_user_id UUID REFERENCES users(id);

ALTER TABLE payments ADD COLUMN IF NOT EXISTS register_id UUID REFERENCES registers(id);

ALTER TABLE cash_drawer_sessions ADD COLUMN IF NOT EXISTS register_id UUID REFERENCES registers(id);
ALTER TABLE cash_drawer_sessions ADD COLUMN IF NOT EXISTS shift_id UUID;
...
--
-- No id column: `locations.id` defaults to uuid_generate_v4(), and this runs
-- once per organisation on an unknown set of orgs, so a fixed id (fine for
-- 015's singleton seed row) would collide on the second row here.
INSERT INTO locations (org_id, name, slug, timezone)
SELECT o.id, 'Main Location', 'main', 'UTC'
...
-- reasoning as 015: neither untargeted ON CONFLICT DO NOTHING nor INSERT OR
-- IGNORE (SQLite side) catches an FK violation, so a data anomaly here would
-- otherwise abort the whole migration and take the container down with it.
INSERT INTO registers (org_id, location_id, name, register_number, display_code, type, status)
SELECT
  o.id,
